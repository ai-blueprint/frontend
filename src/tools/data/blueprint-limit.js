 ...
 */
export function constrainTranslate(x, y, scale, blueprintElement) {
  const containerSize = getContainerSize(blueprintElement);
  const blueprintSize = {
    width: blueprintStore.state.size.width * scale,
    height: blueprintStore.state.size.height * scale
  };
 ...
 * @returns {Object} 限制后的偏移量 {x, y}
 */
export function applyDragConstraints(currentTranslate, offset, currentScale, blueprintElement) {
  // 计算新的偏移量
  const newX = currentTranslate.x + offset.x;
  const newY = currentTranslate.y + offset.y;
  
  // 应用限制
  return constrainTranslate(newX, newY, currentScale, blueprintElement);
}
 ...
 */
export function applyScaleConstraints(currentScale, delta, mouse, currentTranslate, blueprintElement) {
  // 计算新的缩放比例
  ...
  // 计算新的偏移量，保持鼠标位置相对不变
  const newTranslateX = currentTranslate.x + mouse.x * (currentScale - newScale);
  const newTranslateY = currentTranslate.y + mouse.y * (currentScale - newScale);
  
  // 应用偏移限制
  const constrainedTranslate = constrainTranslate(newTranslateX, newTranslateY, newScale, blueprintElement);
