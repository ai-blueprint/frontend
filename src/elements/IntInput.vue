...
<template>
    <div class="int-input"> <!-- 整数输入容器 -->
        <label class="input-label">{{ param.label }}</label> <!-- 参数名称标签 -->
        <div class="input-row"> <!-- 输入行 -->
            <input type="number" class="number-input" :value="currentValue" :min="minValue" :max="maxValue" :step="1" @change="onChange($event.target.value)" /> <!-- 数字输入框 -->
            <input v-if="hasRange" type="range" class="range-slider" :value="currentValue" :min="minValue" :max="maxValue" :step="1" @input="onChange($event.target.value)" /> <!-- 有范围时显示滑块 -->
        </div>
    </div>
</template>
