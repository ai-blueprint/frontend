...
<template>
    <div class="enum-select"> <!-- 枚举选择容器 -->
        ...
        <var-select :model-value="currentValue" @update:model-value="onChange" size="small" variant="outlined"> <!-- Varlet下拉选择组件 -->
            <var-option v-for="option in optionList" :key="option.value" :label="option.label" :value="option.value" /> <!-- 枚举选项 -->
        </var-select>
    </div>
</template>
