...
const toScreen = (canvasX, canvasY) => {
    ...
    const screenX = canvasX * zoom + x                           // 画布X乘缩放加偏移得屏幕X
    const screenY = canvasY * zoom + y                           // 画布Y乘缩放加偏移得屏幕Y
...
const toCanvas = (screenX, screenY) => {
    const { x, y, zoom } = store.viewport                       // 获取视口参数
    const canvasX = (screenX - x) / zoom                         // 屏幕X减偏移除缩放得画布X
    const canvasY = (screenY - y) / zoom                         // 屏幕Y减偏移除缩放得画布Y
